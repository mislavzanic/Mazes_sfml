...
int main(int argc, char **argv) {
    sf::RenderWindow window(sf::VideoMode(760, 760), "MazeGeneration");
    mg::Maze maze(sf::Vector2i(atoi(argv[1]), atoi(argv[2])), sf::Vector2f(760.f, 760.f));
    while (window.isOpen()) {
        sf::Event event;
        ...
        if (!maze.isGenerated) {
            ...
            else if (std::string(argv[3]) == std::string("w"))
                maze.generateWilson(window);
